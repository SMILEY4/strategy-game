...
export class MixedArrayBuffer {
	...
	public setChunk(indexStart: number, type: MixedArrayBufferType, values: number[]) {
		const buffer = this.getBuffer(type);
		buffer.set(values, indexStart);
	}
...
export class MixedArrayBufferCursor {
	...
	private readonly pattern: MixedArrayBufferType[];
	private readonly singlePatternType: MixedArrayBufferType | null; // the single type of the pattern. null if the pattern consists of multiple different types
	...
	constructor(buffer: MixedArrayBuffer) {
		...
		const uniqueTypes = new Set<MixedArrayBufferType>(this.pattern);
		if (uniqueTypes.size === 1) {
			this.singlePatternType = this.pattern[0];
		} else {
			this.singlePatternType = null;
		}
	}

	public appendValues(values: number[]) {

		// optimization for patterns with only a single type
		if (this.singlePatternType !== null) {
			const amountBytes = MixedArrayBuffer.getBytes(this.singlePatternType);
			const index = this.bytePosition / amountBytes;
			this.buffer.setChunk(index, this.singlePatternType, values);
			this.bytePosition += amountBytes * values.length;
		...
		for (let i = 0; i < values.length; i++) {
			...
			const index = this.bytePosition / amountBytes;
			this.buffer.set(index, type, values[i]);
