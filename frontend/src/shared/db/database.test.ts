...
describe("database", () => {
    ...
    describe("subscribers", () => {
        ...
        test("subscribe on entity", () => {
            ...
            db.deleteMany(["2", "3"]) // 4 delete
            db.insertMany([
                {id: "1", size: 1, nested: {value: "a"}},
                {id: "2", size: 2, nested: {value: "b"}}, // 5 insert
                {id: "3", size: 3, nested: {value: "c"}}
            ])
            db.deleteAll() // 6 delete
            ...
            expectEntityCallback(callback.mock.calls[3], "2", DatabaseOperation.DELETE)
            expectEntityCallback(callback.mock.calls[4], "2", DatabaseOperation.INSERT)
            expectEntityCallback(callback.mock.calls[5], "2", DatabaseOperation.DELETE)
        });
        ...
        test("subscribe query", () => {
            ...
            expectQueryCallback(callback.mock.calls[3], [])
            expectQueryCallback(callback.mock.calls[4], ["2", "3", "4"])
            expectQueryCallback(callback.mock.calls[5], [])
        });
    ...
    describe("subscribers with transaction", () => {
        ...
        test("subscribers", () => {
            ...
            expect(callbackOnDb.mock.calls).toHaveLength(2)
            expectDbCallback(callbackOnDb.mock.calls[0], ["1", "2", "2", "3"], DatabaseOperation.DELETE)
            expectDbCallback(callbackOnDb.mock.calls[1], ["2", "1", "2", "3", "1", "2", "3"], DatabaseOperation.INSERT)

            expect(callbackOnEntity.mock.calls).toHaveLength(2)
            expectEntityCallback(callbackOnEntity.mock.calls[0], "2", DatabaseOperation.DELETE)
            expectEntityCallback(callbackOnEntity.mock.calls[1], "2", DatabaseOperation.INSERT)
