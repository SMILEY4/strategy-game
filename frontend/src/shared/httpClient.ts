...
export function isResponseCode(error: ResponseError | any, status: number): boolean {
    return error instanceof ResponseError && error.status === status;
}

export function handleResponseError<T>(error: ResponseError | any, status: number, block: () => T): T {
    if (isResponseCode(error, status)) {
        return block();
    }
    throw error;
}