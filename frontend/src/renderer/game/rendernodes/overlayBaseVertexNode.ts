...
export class OverlayBaseVertexNode extends VertexRenderNode<GameWebGLRenderContext> {

	public static readonly ID = "vertexnode.overlay.base";
	...
	constructor() {
		super({
			id: OverlayBaseVertexNode.ID,
			changeKey: OverlayBaseVertexNode.ID,
			input: [],
			output: [
				new VertexBuffer({
	...
	public execute(context: GameWebGLRenderContext): VertexDataResource {
		...
		return new VertexDataResource({
			buffers: buildMap({
				"vertexbuffer.mesh.overlay": new VertexBufferResource(baseMeshData),
			}),
			outputs: emptyMap(),
		});
	}
	...
	private buildBaseMesh(): [number, ArrayBuffer] {
		const [arrayBuffer, cursor] = MixedArrayBuffer.createWithCursor(OverlayBaseVertexNode.MESH_VERTEX_COUNT, OverlayBaseVertexNode.MESH_PATTERN);
		...
		return [OverlayBaseVertexNode.MESH_VERTEX_COUNT, arrayBuffer.getRawBuffer()];
	}
