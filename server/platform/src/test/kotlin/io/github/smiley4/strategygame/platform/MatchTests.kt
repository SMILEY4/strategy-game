...
class MatchTests : FreeSpec({
    ...
    "create" - {
        "should successfully create and save a match" {
            ...
            val service = MatchServiceImpl(mockk<GameEngineClient>(), repository)
    ...
    "join" - {
        ...
        "should fail when trying to join a match after game generation" {
            ...
            val service = MatchServiceImpl(gameEngineClient, repository)
    ...
    "generateGame" - {
        "should create a game through the engine" {
            val repository = InMemoryMatchRepository()
            val gameEngine = mockk<GameEngineClient>()
            val service = MatchServiceImpl(gameEngine, repository)

            val ownerId = UserId()
            val matchId = service.create(ownerId, "Ready Match")

            val gameId = GameId()
            every { gameEngine.createGame(any()) } returns gameId

            service.generateGame(ownerId, matchId)

            verify(exactly = 1) { gameEngine.createGame(any()) }
            repository.findById(matchId) shouldNotBe null
            repository.findById(matchId)?.toSnapshot()?.also {
