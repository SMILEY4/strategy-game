use crate::js::models::{Control, Entity, HexPosition, SpriteSheetEntry, Tile, MAP_MODE_TERRAIN, MAP_MODE_POLITICAL, MAP_MODE_SETTLEMENT_LOCATIONS};
...
use crate::render::models::tile_instance_data::{
    ...
    MapDetailsVertexData, OverlayVertexData, TileFogOfWarInstance, TileInstanceData,
    TileTerrainLandInstance, TileTerrainWaterInstance,
};
use crate::render::{creator_map_detail_instances, creator_overlay_instances, creator_terrain_tile_instances, overlay_functions};
...
impl Renderer {
    ...
    pub fn build_overlay_instances(&mut self) {
        ...
        let has_selected_entity = self.state.selected_entity_id.is_some();

        let create_fill = move |state: &RenderState, tile: &Tile, output: &mut Vec<GenericFillOverlayInstance>| {
            match map_mode {
                MAP_MODE_TERRAIN => overlay_functions::fill_mapmode_terrain(state, tile, output),
                MAP_MODE_POLITICAL => overlay_functions::fill_mapmode_political(state, tile, output),
                MAP_MODE_SETTLEMENT_LOCATIONS => overlay_functions::fill_mapmode_settlement_locations(state, tile, output),
                _ => overlay_functions::fill_none(state, tile, output),
            }
        };

        let create_edge = move |state: &RenderState, tile: &Tile, tiles_by_pos: &FxHashMap<HexPosition, usize>, output: &mut Vec<GenericEdgeOverlayInstance>| {
            match map_mode {
                MAP_MODE_TERRAIN => overlay_functions::edges_mapmode_terrain(state, tile, tiles_by_pos, output),
                MAP_MODE_POLITICAL => overlay_functions::edges_mapmode_political(state, tile, tiles_by_pos, output),
                MAP_MODE_SETTLEMENT_LOCATIONS => overlay_functions::edges_mapmode_settlement_locations(state, tile, tiles_by_pos, output),
    ...
    pub fn calculate_instances(&mut self) {
        ...
        creator_map_detail_instances::build(
            &self.config,
            &self.state,
            &mut self.map_detail_vertex_data,
        );
    }
