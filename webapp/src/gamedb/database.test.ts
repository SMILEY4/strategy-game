...
import type {Database} from "@gamedb/database/database.ts";
import {DatabaseOperation} from "@gamedb/database/database-operation.ts";
...
type TestStorageMapping = {
    primary: MapPrimaryDatabaseStorageUnit<TestEntity, string>,
}

type TestQuery<ARGS> = Query<TestStorageMapping, TestEntity, string, ARGS>

const QUERY_SIZE_GREATER_OR_EQUAL: TestQuery<number> = {
    run(storage: TestStorageMapping, args: number): TestEntity[] {
        return storage.primary.getAll().filter(e => e.size >= args).sort((a, b) => a.size - b.size);
    },
};
...
describe("database", () => {
    ...
    describe("crud", () => {
        ...
        test("insert single", () => {
            ...
            expect(db.queryById("1")?.nested.value).toBe("a");

            const id2 = db.insert({id: "2", size: 2, nested: {value: "b"}});
            expect(id2).toBe("2");
            expect(db.count()).toBe(2);
            expect(db.queryById("1")?.nested.value).toBe("a");
            expect(db.queryById("2")?.nested.value).toBe("b");
        ...
        test("delete single", () => {
            ...
            expect(db.count()).toBe(1);
            expect(db.queryById("1")).not.toBe(null);
            expect(db.queryById("2")).toBe(null);
        ...
        test("delete many", () => {
            ...
            expect(db.count()).toBe(1);
            expect(db.queryById("1")).toBe(null);
            expect(db.queryById("2")).not.toBe(null);
            expect(db.queryById("3")).toBe(null);
        ...
        test("update single", () => {
            ...
            expect(db.queryById("2")?.id).toBe("2");
            expect(db.queryById("2")?.size).toBe(12);
            expect(db.queryById("2")?.nested?.value).toBe("b");
        ...
        test("update many", () => {
            ...
            expect(db.queryById("2")?.size).toBe(12);
            expect(db.queryById("2")?.nested?.value).toBe("b");
            expect(db.queryById("3")?.id).toBe("3");
            expect(db.queryById("3")?.size).toBe(13);
            expect(db.queryById("3")?.nested?.value).toBe("c");
        ...
        test("replace single", () => {
            ...
            expect(db.queryById("2")?.id).toBe("2");
            expect(db.queryById("2")?.size).toBe(12);
            expect(db.queryById("2")?.nested?.value).toBe("x");
        ...
        test("replace many", () => {
            ...
            expect(db.queryById("3")?.id).toBe("3");
            expect(db.queryById("3")?.size).toBe(13);
            expect(db.queryById("3")?.nested?.value).toBe("x");
    ...
    describe("subscribers with transaction", () => {
        ...
        test("subscribers", () => {
            ...
            const callbackOnDb = vi.fn();
            db.subscribe(callbackOnDb);

            const callbackOnEntity = vi.fn();
            db.subscribeOnEntity("2", callbackOnEntity);

            const callbackOnQuery = vi.fn();
