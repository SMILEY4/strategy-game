import type {SingleDatabaseStorageResult} from "@gamedb/storage/database-storage-result.single.ts";
import type {ManyDatabaseStorageResult} from "@gamedb/storage/database-storage-result.many.ts";
import type {ModifyDatabaseStorageResult} from "@gamedb/storage/database-storage-result.modify.ts";
import {type IdProvider, IdProviderUtils} from "@gamedb/storage/id-provider.ts";
import type {PrimaryDatabaseStorageUnit} from "@gamedb/storage/database-storage-unit.primary.ts";

/**
 * Maps a unique id to a unique entity
 */
export class MapPrimaryDatabaseStorageUnit<ENTITY, ID> implements PrimaryDatabaseStorageUnit<ENTITY, ID> {
    ...
    public deleteAll(): ManyDatabaseStorageResult<ENTITY, ID> {
        ...
        const ids = IdProviderUtils.toIds(this.idProvider, entities);
        this.entities.clear();
