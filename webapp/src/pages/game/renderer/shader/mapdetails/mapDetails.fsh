...
void main() {
    ...
    vec3 colorBaseLandLight = vec3(103.0, 140.0, 52.0) / 255.0;
    vec3 colorBaseLandDark = vec3(90.0, 127.0, 39.0) / 255.0;
    vec3 colorBaseLand = colorBaseLandLight; //mix(colorLight, colorDark, (random(v_tilePosition) + 1.0) * 0.5);
    ...
    spriteColor = mix(spriteRawColor, vec4(colorBaseLand, 1.0) * spriteRawColor , spriteMask.r);
