...
export type ShaderUniformValues = Map<string, number[] | number | Float32Array> | ShaderUniformValuesObject

export interface ShaderUniformValuesObject {
    [key: string]: number[] | number | Float32Array;
}

export type ShaderAttributeBuffers = Map<string, WebGLBuffer | GLBuffer> | ShaderAttributeBuffersObject

export interface ShaderAttributeBuffersObject {
    [key: string]: WebGLBuffer | GLBuffer;
}

export interface ShaderRuntimeData {
    attributeBuffers?: ShaderAttributeBuffers,
...
export class ShaderProgram {
    ...
    /**
     * @return the names of all attributes in order
     */
    public getAttributes(): string[] {
        return this.data.attributes.map(a => a.name);
    }
    ...
    public use(data: ShaderRuntimeData) {
        if (this.programHandle != null) {
            ...
            this.setAttributes(this.data.attributes, this.attributeLocations, attributeBufferMap);
            // set uniform values
            ...
            this.setUniforms(this.data.uniforms, this.uniformLocations, uniformValuesMap);
        } else {
    ...
    private setAttributes(attributes: ShaderAttributeData[], locations: Map<string, GLint>, buffers: Map<string, WebGLBuffer | GLBuffer>) {
        // calculate the total "offset" between each vertex in bytes
        const strideBytes = attributes
            .map(a => a.amountComponents * ShaderProgram.shaderAttributeTypeToBytes(a.type))
            .reduce((a, b) => a + b, 0);
        // set each attribute
        let offsetBytes = 0;
        attributes.forEach(attribute => {
            if (locations.has(attribute.name) && buffers.has(attribute.name)) {
                this.setAttributeValue(attribute, buffers.get(attribute.name)!, strideBytes, offsetBytes);
            } else {
                console.warn("No location or buffer found for attribute " + attribute.name + " and shader " + this.data.debugName);
            }
            offsetBytes += attribute.amountComponents * ShaderProgram.shaderAttributeTypeToBytes(attribute.type);
        });
    }


    private setAttributeValue(attribute: ShaderAttributeData, buffer: WebGLBuffer | GLBuffer, stride: number, offset: number) {
    ...
    private setUniforms(uniforms: ShaderUniformData[], locations: Map<string, WebGLUniformLocation>, values: Map<string, number[] | number>) {
        uniforms.forEach((uniform) => {
            if (locations.has(uniform.name)) {
                const value = values.has(uniform.name) ? values.get(uniform.name)! : uniform.defaultValue;
                if (value !== null && value !== undefined) {
                    this.setUniformValue(uniform, this.getUniformValue(value));
                }
            }
        });
    }
    ...
    private getUniformValue(value: any): number[] {
